...
public final class DexFile1 {
    ...
    /**
     * Opens a DEX file from a given filename, using a specified file     * to hold the optimized data.     *     * @param sourceName     *  Jar or APK file with "classes.dex".     * @param outputName     *  File that will hold the optimized form of the DEX data.     * @param flags     *  Enable optional features.
     */
    private DexFile1(String sourceName, String outputName, int flags) throws IOException {
        if (outputName != null) {
            try {
            ...
            } catch (ErrnoException ignored) {                // assume we'll fail with a more contextual error later
            }
        }
     ...
     * * regenerate it.     *     * This is intended for use by applications that wish to download
     ...
     * * @return     *  A new or previously-opened DexFile1.
     * * @throws IOException     *  If unable to open the source or output file.
     */
    static public DexFile1 loadDex(String sourcePathName, String outputPathName, int flags) throws IOException {        /*         * TODO: we may want to cache previously-opened DexFile1 objects.         * The cache would be synchronized with close().  This would help         * us avoid mapping the same DEX more than once when an app         * decided to open it multiple times.  In practice this may not         * be a real issue.         */
        return new DexFile1(sourcePathName, outputPathName, flags);
    }

    /**
     * Gets the name of the (already opened) DEX file.     *     * @return the file name
     */
    ...
    /**
     * Closes the DEX file.     * <p>     * This may not be able to release any resources. If classes from this     * DEX file are still resident, the DEX file can't be unmapped.     *     * @throws IOException     *             if an I/O error occurs during closing the file, which     *             normally should not happen
     */
    ...
    /**
     * Loads a class. Returns the class on success, or a {@code null} reference     * on failure.     * <p>
     *     * If you are not calling this from a class loader,
     *     this is most likely not     * going to do what you want.
     *     Use {@link Class#forName(String)} instead.     * <p>
     ...
     *         *     * @param name     *            the class name, which should look like "java/lang/String"     *
     *         * @param loader     *            the class loader that tries to load the class (in most cases     *
     *         the caller of the method     *     * @return the {@link Class} object representing the class, or {@code null}     *
     ...
     * * This takes a "binary" class name to better match ClassLoader semantics.     *     * @hide
     */
    ...
    /**
     * Called when the class is finalized. Makes sure the DEX file is closed.     *     * @throws IOException     *             if an I/O error occurs during closing the file, which     *             normally should not happen
     */
    @Override
